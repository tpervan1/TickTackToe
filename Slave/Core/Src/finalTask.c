  ...
  if (msg.header[0] == 'I' && msg.header[1] == ':') {
	  ...
	  if(msg.payload[0] == '2') {
			...
			if(1){
				...
				*currentState = SEND_ID; 
				...
				printf(" Received valid id, send slave id! ");
	...
	// K message check for ID exchange
		...
		*currentState = GAME_START;
		...
		printf(" Successfull ID transmission to master, GAME_START!");
	}
	
	else if (msg.header[0] == 'K' && msg.header[1] == ':' && msg.payload[0] == 'T') {
		printf(" Sent table was valid, waiting master response");
	}
		...
		*currentState = IDLE;
		...
		//check validity and sign placement and CRC
	  ...
	  uint8_t receivedTable[9];
		...
		*currentState = IDLE;
		...
		//print game result
		//have to change this so it includes D
...
void vCheckUserInputRST(linear_buf lb, GameState *currentState) {
	//printf("%d", strncmp((char*)lb.buf, "RST:", 4));
	//vDelay(1);
